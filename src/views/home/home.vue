...
export default {
    ...
    created () {
        this.$store.dispatch('GetUserInfo').then(res => {
            ...
            if (res.data.result.roles[0].name == 'litigant') {
    ...
    // 案件数统计及环形图
        ...
        schedulingCase().then(res => {
            ...
            '日'; // 日期
                ...
                } else if (allAry.length > 6) {
                    if (allAry.length > 6 && allAry.length <= 12) {
                        for (var j = 0; j < newDateAry.length; j++) {
                            var dateSmallAry = [];
                            var fi = 1;
                            for (var i = 0; i < 6; i++) {
                        ...
                        for (var j = 0; j < newDateAry.length; j++) {
                            var dateSmallAry = [];
                            var fi = 1;
                            for (var i = 12; i < allAry.length; i++) {
                                if (allAry[i].date == newDateAry[j]) {
                                    fi = 0;
                                    dateSmallAry.push(allAry[i]);
                                }
